...
const STATUS_CLASS={APROVADA_AUTO:'ok',PROCESSADO:'ok',INTEGRADO:'ok',AMBIGUO:'warn',REVISAO:'warn',CONCORRENCIA:'warn',ERRO:'error'};
const HINTS={
...
function cell(key,value){if((key==='status'||key==='sync_status')&&value){return `<span class="pill ${STATUS_CLASS[value]||'neutral'}">${esc(value)}</span>`}const text=esc(value);return key==='criterios'||key==='divergencias'||key==='response_summary'||key==='last_error'?`<span class="wrap">${text}</span>`:text}
...
function url(){if(state.view==='logs'){const p=new URLSearchParams({limit:'300'});if($('#logType').value)p.set('tipo',$('#logType').value);if($('#logErrors').checked)p.set('erros','true');return `/api/hub-crm/logs?${p}`}return `/api/hub-crm/${state.view}?limit=200`}
async function table(){try{state.rows=await request(url());render()}catch(e){message(e.message,true)}}
function toolbar(){const logs=state.view==='logs';$('#logType').hidden=!logs;$('#logErrorsWrap').hidden=!logs;$('#approveNow').hidden=state.view!=='aprovacoes-cte'||!state.autoApproval;$('#hint').textContent=HINTS[state.view]||''}
...
document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));b.classList.add('active');state.view=b.dataset.view;$('#search').value='';toolbar();table()});

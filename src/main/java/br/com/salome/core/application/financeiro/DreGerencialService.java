...
/**
 * Monta o DRE gerencial de caixa sobre a estrutura real do plano de contas legado (classificacao
 * hierarquica, sintetico/analitico e a flag impostosFinanceiro), em vez do antigo de-para por
 * palavra-chave. Cada despesa entra na secao fiel ao legado e mantem a origem (Nota de compra,
 * Extrato, Caixa) no detalhe analitico.
 */
@Service
public class DreGerencialService {
    ...
    @Autowired
    public DreGerencialService(FinanceiroFluxoCaixaRepository repository) {
        this(repository, Clock.systemDefaultZone());
    }

    DreGerencialService(FinanceiroFluxoCaixaRepository repository, Clock clock) {
    ...
    /** Drill-down read-only: CT-es que compoem uma fatura (nivel abaixo do card FAT-). */
    public List<DreFaturaCte> ctesDaFatura(int idFatura) {
        return repository.listarCtesDetalhadosDaFatura(idFatura);
    }
    ...
    public DreGerencialSnapshot dashboard(FinanceiroFiltro filtro, Integer filialId, String filial) {
        ...
        List<FinanceiroMovimento> todos = repository.listarMovimentos(normalizado).stream()
                .filter(movimento -> movimento.status() == FinanceiroStatus.REALIZADO)
                .filter(movimento -> dentroPeriodoBaixa(movimento, normalizado))
        ...
        return new DreGerencialSnapshot(
                ...
                movimentos.stream()
                        .sorted(Comparator.comparing(FinanceiroMovimento::dataBaixa, Comparator.nullsLast(Comparator.naturalOrder()))
    ...
    // ------------------------------------------------------------------------------------------------
    // Classificacao por secao (fiel ao legado)
    // ------------------------------------------------------------------------------------------------
    ...
    /**
     * Secao a partir da descricao do grupo sintetico do plano de contas (ex.: "2.08 CUSTOS
     * OPERACIONAIS COM FROTA" -> Custos). Ancorar no nome real do plano evita o "sem mapeamento"
     * do antigo de-para por palavra-chave de historico.
     */
    ...
    /** Grupo sintetico de 2o nivel da classificacao (ex.: "2.08.003" -> "2.08"). */
    ...
    /** Sentinelas (~) vao para o fim da secao; demais pela ordem natural da classificacao. */
